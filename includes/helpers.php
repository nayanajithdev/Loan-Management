...
function customer_list_rows(PDO $pdo, ?array $viewer = null, string $searchTerm = ''): array
{
    $viewer = $viewer ?? current_user();
    $viewerRole = (string) ($viewer['role'] ?? '');
    $viewerId = (int) ($viewer['id'] ?? 0);
    $canViewAll = in_array($viewerRole, ['superadmin', 'admin', 'collector_l2', 'collector'], true);
    ...
    if ($canViewAll) {
        $sql = $selectBase . " WHERE 1=1" . ($searchTerm !== '' ? $searchClause : '') . " ORDER BY c.id DESC";
    ...
    if ($searchTerm !== '') {
        $searchValue = '%' . escape_like_value($searchTerm) . '%';
